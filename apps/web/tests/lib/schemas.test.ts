...
describe("foundational API schemas", () => {
  ...
  it("parses the complete competitor read contract", () => {
    ...
    expect(() => competitorSchema.parse({ ...competitor, status: "unknown" })).toThrow(
      z.ZodError,
    );
    expect(() =>
      competitorSchema.parse({ ...competitor, daily_run_time_local: "8:30" }),
    ).toThrow(z.ZodError);
  });
  ...
  it("parses the complete monitored-source read contract", () => {
    ...
    expect(() => sourceSchema.parse({ ...source, source_category: "social" })).toThrow(
      z.ZodError,
    );
    expect(() => sourceSchema.parse({ ...source, created_at: "yesterday" })).toThrow(
      z.ZodError,
    );
  });
});
...
describe("audit schemas", () => {
  it("parses finding decimals and strips unknown model fields", () => {
    const parsed = findingSchema.parse({
      ...
      title: "Pricing changed", summary: "Summary", significance_explanation: "Explanation",
      significance_level: "high", confidence: "0.91", decision_rationale: "Rationale",
      first_seen_at: "2026-08-21T08:00:00Z", last_seen_at: "2026-08-21T09:00:00Z",
      published_at: "2026-08-21T09:00:00Z", prompt: "must be stripped",
    });
  ...
  it("rejects unsafe evidence URLs and strips task secrets", () => {
    expect(() => findingEvidenceSchema.parse({
      id: "66666666-6666-4666-8666-666666666666", source_url: "javascript:alert(1)",
      source_domain: "example.com", source_title: "Source", source_type: "first_party", published_at: null,
      captured_at: "2026-08-21T08:00:00Z", quoted_text: "Quote", normalized_claim: "Claim",
    ...
    const task = agentTaskSchema.parse({
      id: "77777777-7777-4777-8777-777777777777", scout_run_id: "44444444-4444-4444-8444-444444444444",
      parent_task_id: null, role: "child_researcher", task_kind: "pricing", status: "succeeded",
      model_alias: "research", objective: "Review pricing", source_scope: ["https://example.com"],
      attempt_count: 1, started_at: null, completed_at: null, input_tokens: 0, output_tokens: 0,
      tool_calls: null, settled_cost_usd: null, validated_output: null, error_code: null, error_summary: null,
      created_at: "2026-08-21T08:00:00Z", prompt: "secret", raw_response: "secret",
    });
  ...
  it("validates run lifecycle enums", () => {
    const base = {
      id: "44444444-4444-4444-8444-444444444444", competitor_id: null, run_type: "daily_scout",
      scheduled_for: "2026-08-21T08:00:00Z", started_at: null, completed_at: null,
      failure_code: null, failure_summary: null, partial_reasons: [], input_tokens: 0, output_tokens: 0,
      tool_calls: null, settled_cost_usd: null, created_at: "2026-08-21T08:00:00Z",
    };
...
describe("brief, settings, and aggregate usage schemas", () => {
  it("validates grounded and deterministic empty weekly briefs", () => {
    ...
    expect(weeklyBriefSchema.parse({
      ...base,
  ...
  it("accepts only public settings and preserves unknown usage totals", () => {
    ...
    expect(usageSummarySchema.parse({ items: [{
      date: "2026-08-21",
    ...
    }] }).items[0]).toMatchObject({ tool_calls: null, settled_cost_usd: null });
  });
});
