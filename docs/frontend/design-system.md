...
Before the first published digest, never present a generic empty report. `DigestStatusCard` translates the backend overview into five explicit states: setup required, setup incomplete, first scan running, awaiting the first scheduled digest, and archive available. Waiting shows the backend-provided user-local generation time, active competitor and approved-source scope, snapshot links, and monitoring issues; paused setup has no future timestamp until monitoring resumes. Existing archive history has priority over onboarding guidance, and the dashboard uses the same overview facts in compact context.

A quiet week is a successful monitoring result with the canonical title `No important changes found this week`, not an absent report. Every digest detail includes `MonitoringCoverageReceipt`, collapsed to a complete/incomplete treatment and expandable to immutable completed, partial, failed, successfully inspected-source, and covered-competitor facts. Partial or failed scans keep a warning visible without relying on color. Historical digests with `NULL` coverage show a non-numeric explanation rather than zeros or reconstructed current state.
...
A section's name is a proper noun and keeps title case everywhere it names the section, including inside a sentence: `Weekly Digest`. Common nouns for what a section contains stay lowercase in prose, as in "backed by the updates behind it" or "the scan that produced this digest". Persisted result titles such as `No important changes found this week` use sentence case. Sentence case remains the default for every other heading, label, button, and state.
